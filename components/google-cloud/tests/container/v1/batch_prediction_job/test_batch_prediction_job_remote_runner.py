...
class BatchPredictionJobRemoteRunnerUtilsTests(
  ...
  def setUp(self):
    ...
    self._payload = json.dumps({
        ...
        'inputConfig': {
            'instancesFormat': 'CSV',
            'gcsSource': {'uris': ['test_gcs_source']},
        },
        'outputConfig': {
            'predictionsFormat': 'CSV',
            'gcsDestination': {'outputUriPrefix': 'test_gcs_destination'},
        },
    })
    ...
    self._batch_prediction_job_name = (
        '/projects/{self._project}/locations/{self._location}/jobs/test_job_id'
    )
    ...
    self._executor_input = (
        ...
        '     '
        ' "gcs_output_directory":{"artifacts":[{"metadata":{},"name":"gcs_output","type":{"schemaTitle":"system.Artifact"},"uri":"gs://abc"}]}},"outputFile":"'
  ...
  def test_batch_prediction_job_remote_runner_succeeded_output_bq_table(
    ...
    job_client.create_batch_prediction_job.return_value = (
        create_batch_prediction_job_response
    )
    create_batch_prediction_job_response.name = self._batch_prediction_job_name

    get_batch_prediction_job_response = mock.Mock()
    job_client.get_batch_prediction_job.return_value = (
        get_batch_prediction_job_response
    )
    ...
    get_batch_prediction_job_response.output_info.bigquery_output_dataset = (
        'bq://bq_project.bigquery_output_dataset'
    )
    ...
    with open(self._gcp_resources) as f:
      ...
      batch_prediction_job_name = batch_prediction_job_resources.resources[
          0
      ].resource_uri[len(self._batch_prediction_job_uri_prefix) :]
      self.assertEqual(
    ...
    with open(self._output_file_path) as f:
      ...
      self.assertEqual(
          executor_output,
          json.loads(
              '{"artifacts": {              "batchpredictionjob": {"artifacts":'
              ' [{"metadata": {"resourceName": "job1", "bigqueryOutputDataset":'
              ' "bq://bq_project.bigquery_output_dataset","bigqueryOutputTable":'
              ' "bigquery_output_table","gcsOutputDirectory": ""}, "name":'
              ' "foobar", "type": {"schemaTitle":'
              ' "google.VertexBatchPredictionJob"}, "uri":'
              ' "https://test_region-aiplatform.googleapis.com/v1/job1"}]},    '
              '          "bigquery_output_table": {"artifacts": [{"metadata":'
              ' {"projectId": "bq_project", "datasetId":'
              ' "bigquery_output_dataset", "tableId": "bigquery_output_table"},'
              ' "name": "bq_table", "type": {"schemaTitle": "google.BQTable"},'
              ' "uri":'
              ' "https://www.googleapis.com/bigquery/v2/projects/bq_project/datasets/bigquery_output_dataset/tables/bigquery_output_table"}]}}}'
          ),
      )
  ...
  def test_batch_prediction_job_remote_runner_job_exists(
    ...
    remote_runner = job_remote_runner.JobRemoteRunner(
        type, self._project, self._location, self._gcp_resources
    )
  ...
  def test_batch_prediction_job_remote_runner_succeeded_output_gcs(
    ...
    with open(self._output_file_path) as f:
      ...
      self.assertEqual(
          executor_output,
          json.loads(
              '{"artifacts": {                "batchpredictionjob":'
              ' {"artifacts": [{"metadata": {"resourceName": "job1",'
              ' "bigqueryOutputTable": "", "bigqueryOutputDataset": "",'
              ' "gcsOutputDirectory": "gs://foo_gcs_output_directory"}, "name":'
              ' "foobar", "type": {"schemaTitle":'
              ' "google.VertexBatchPredictionJob"}, "uri":'
              ' "https://test_region-aiplatform.googleapis.com/v1/job1"}]},    '
              '            "gcs_output_directory": {"artifacts": [{"metadata":'
              ' {}, "name": "gcs_output", "type": {"schemaTitle":'
              ' "system.Artifact"}, "uri": "gs://foo_gcs_output_directory"}]}}}'
          ),
      )
  ...
  def test_batch_prediction_job_remote_runner_retries_to_get_status_on_non_completed_job(
    ...
    get_batch_prediction_job_response_success.state = (
        gca_job_state.JobState.JOB_STATE_SUCCEEDED
    )
    get_batch_prediction_job_response_success.output_info.bigquery_output_table = (
        'bigquery_output_table'
    )
    get_batch_prediction_job_response_success.output_info.bigquery_output_dataset = (
        'bq://bq_project.bigquery_output_dataset'
    )
    get_batch_prediction_job_response_success.output_info.gcs_output_directory = (
        'gcs_output_directory'
    )

    get_batch_prediction_job_response_running = mock.Mock()
    get_batch_prediction_job_response_running.state = (
